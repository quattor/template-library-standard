...
# Account parameters is a dict where valid entries are ncm-accounts
# properties for users. Ensure a shell is defined.

variable USER_LIST ?= dict(
#    'userex', dict('uid', 711,
#                    'groups', list('groupex'),
#                   ),
);
...
variable USER_SSH_KEYS ?= dict(
#    'userex', USER_LIST_SSHKEY_URL_PREFIX + 'userex.pub',
);

# Nlist of group to create. Value is a dict where entries must be a
# valid property for ncm-accounts groups.
variable GROUP_LIST ?= dict(
#    'groupex', dict('gid', 800),
);
...
variable DB_MACHINE_GROUPS ?= dict(
...
variable DB_MACHINE_GROUPS_ONLY ?= dict(
);

# Per-machine list of users to create.
# Key is a machine name, value is a list of userids.
variable DB_MACHINE_USERS ?= dict(
);

# Per-machine list of group memberships
# Key is a machine name, value is a dict with a username as key and a list of groups as value.
# variable DB_MACHINE_USERS_GROUPS = dict(
#   'mynode.example.com', dict('userex', list('groupex1','groupex2')),
# );
variable DB_MACHINE_USERS_GROUPS ?= dict(
);
...
# Key is the regexp, value is a dict which must contain a key 'entries' that will
...
variable DB_MACHINE_DEFAULT_ENTRIES ?= dict(
);
...
variable NODE_USER_GROUP_ENTRIES = {
    group_entries = dict(FULL_HOSTNAME, '');
    user_entries = dict(FULL_HOSTNAME, '');
    ...
    foreach (e_regexp; params; DB_MACHINE_DEFAULT_ENTRIES) {
        ...
        if ( exists(params['entries']) ) {
            if ( is_list(params['entries']) ) {
                ...
                error("Invalid value for DB_MACHINE_DEFAULT_ENTRIES entry "
                    + regexp + ": 'entries' must be a string or list of string");
            };
            if ( match(FULL_HOSTNAME, regexp) ) {
                always_add = exists(params['alwaysAdd']) && is_defined(params['alwaysAdd']) && params['alwaysAdd'];
                # Insert into dict first to eliminate duplicates.
                ...
                foreach (i; entry; entries) {
                    ...
                    if ( !exists(DB_MACHINE_USERS[entry]) && !exists(DB_MACHINE_GROUPS[entry]) ) {
                        error('Entry ' + entry + ' referenced by DB_MACHINE_DEFAULT_ENTRIES['
                            + regexp + '] does not exist neither in DB_MACHINE_GROUPS nor in DB_MACHINE_USERS');
                    };
...
variable NODE_USER_GROUP_LIST = {
    ...
    # Construct a list of groups without duplicates (dict)
        ...
        if ( exists(DB_MACHINE_GROUPS[entry]) ) {
            foreach (i; group; DB_MACHINE_GROUPS[entry]) {
                if ( exists(GROUP_LIST[group]) ) {
                    ...
                    error('Group ' + group + ' referenced by DB_MACHINE_GROUPS entry '
                        + entry + ' but not defined in GROUP_LIST');
                };
            };
        };
    };
    ...
    # Add to the list of users, all users explicitly configured on this machine without duplicates (dict)
        ...
        if ( exists(DB_MACHINE_USERS[entry]) ) {
            foreach (i; user; DB_MACHINE_USERS[entry]) {
                if ( exists(USER_LIST[user]) ) {
                    ...
                    error('User ' + user + ' referenced by DB_MACHINE_USERS entry '
                        + entry + ' but not defined in USER_LIST');
                };
            };
        };
    };

    SELF['missing_users_groups'] = dict();
    ...
    foreach (e_regexp; params; DB_MACHINE_DEFAULT_ENTRIES) {
        ...
        if ( exists(params['entries']) ) {
            if ( is_list(params['entries']) ) {
                ...
                error("Invalid value for DB_MACHINE_DEFAULT_ENTRIES entry " + regexp +
                    ": 'entries' must be a string or list of string");
            };
            if ( match(FULL_HOSTNAME, regexp) ) {
                foreach (i; entry; entries) {
...
'/software/components/accounts' = {
    ...
    if ( length(NODE_USER_GROUP_LIST['groups']) > 0 ) {
        account_config = create_accounts_from_db(GROUP_LIST, NODE_USER_GROUP_LIST['groups'], 1);
    };

    if ( length(NODE_USER_GROUP_LIST['users']) > 0 ) {
        user_config = create_accounts_from_db(USER_LIST, NODE_USER_GROUP_LIST['users']);
    };
